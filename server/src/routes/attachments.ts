...
async function verifyEntityOwnership(entityType: string, entityId: string, userId: number): Promise<boolean> {
  ...
  switch (entityType) {
    case 'home': {
      const rows = await sql`SELECT id FROM homes WHERE id = ${numId} AND user_id = ${userId}`;
      return rows.length > 0;
    }
    case 'room': {
      const rows = await sql`SELECT r.id FROM rooms r JOIN homes h ON r.home_id = h.id WHERE r.id = ${numId} AND h.user_id = ${userId}`;
      return rows.length > 0;
    }
    case 'project': {
      const rows = await sql`SELECT p.id FROM projects p JOIN homes h ON p.home_id = h.id WHERE p.id = ${numId} AND h.user_id = ${userId}`;
      return rows.length > 0;
    }
    case 'item': {
      const rows = await sql`SELECT i.id FROM items i JOIN rooms r ON i.room_id = r.id JOIN homes h ON r.home_id = h.id WHERE i.id = ${numId} AND h.user_id = ${userId}`;
      return rows.length > 0;
    }
    default:
      return false;
...
router.get('/', wrap(async (req, res) => {
  ...
  const attachments = await sql`
    ...
    WHERE (${hId === null} OR home_id = ${hId})
      AND (${rId === null} OR room_id = ${rId})
      AND (${pId === null} OR project_id = ${pId})
      AND (${iId === null} OR item_id = ${iId})
...
router.delete('/:id', wrap(async (req, res) => {
  ...
  const sql = getDb();
  await sql`DELETE FROM attachments WHERE id = ${Number(req.params.id)}`;
